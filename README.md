...
**Hero reveal.** Moving the mouse over the hero uncovers the magnetic survey image in small square blocks that fade back out. On phones and tablets it follows the finger as the visitor swipes across the hero, with a slightly larger brush and a longer linger, and never blocks the page from scrolling. It is switched off for visitors who have "reduce motion" enabled. The settings are data attributes on the hero `<section>`:
...
- The card spotlight highlights only run with a mouse. The hero reveal runs with a mouse or a finger.
- The contour texture behind the page stays fixed while scrolling on desktop and mobile. On touch screens it is sized to the large viewport so it never re-scales as the browser's address bar shows or hides.
