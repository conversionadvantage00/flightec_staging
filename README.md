...
- Fixed, contour-line background texture behind all content sections,
  with separate light/dark-theme images on desktop and separate,
  higher-resolution mobile crops (also light/dark) so it doesn't
  pixelate on phone screens. It's now built as a real background
  layer in the page (not a CSS trick some phone browsers didn't
  render), and the texture now reads consistently behind card content
  too, not just in the gaps between cards — both of those were fixed
  in this update.
